...
public class LargestArmyTest {
    ...
    public void testLargestArmy() throws Exception {
        ...
        m_game.getPlayer(0).giveSoldier();
        m_game.getPlayer(0).giveSoldier();
        m_game.getPlayer(2).giveSoldier();
        m_game.calculateVictoryPoints();
        assertNull("No one should has 3 soldiers yet.", m_game.getLargestArmy());

        m_game.getPlayer(0).giveSoldier();
        m_game.calculateVictoryPoints();
        assertEquals(m_game.getPlayer(0), m_game.getLargestArmy());

        // steal the largest army from player 0 with four soldiers
        m_game.getPlayer(1).giveSoldier();
        m_game.getPlayer(1).giveSoldier();
        m_game.getPlayer(1).giveSoldier();
        m_game.calculateVictoryPoints();
        assertEquals(m_game.getPlayer(0), m_game.getLargestArmy());
        ...
        m_game.getPlayer(1).giveSoldier();
        m_game.calculateVictoryPoints();
        assertEquals(m_game.getPlayer(1), m_game.getLargestArmy());
        ...
        m_game.getPlayer(3).giveSoldier();
        m_game.getPlayer(3).giveSoldier();
        m_game.getPlayer(3).giveSoldier();
        m_game.getPlayer(3).giveSoldier();
        m_game.getPlayer(3).giveSoldier();
        m_game.getPlayer(3).giveSoldier();
        m_game.calculateVictoryPoints();
        assertEquals("Player 3 should now have largest army with 6", m_game.getPlayer(3), m_game.getLargestArmy());
    }
}
